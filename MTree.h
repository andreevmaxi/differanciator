...
bool MBinaryTree_t::DeleteNode(MNode_Binar_t* DelNode, MNode_Binar_t** LeftSubTree, MNode_Binar_t** RightSubTree)
    ...
    void MBinaryTree_t::TDUMP(int err)
        ...
        while(!NodesToGo.empty())
            ...
            if(NowNode->Mode == NUM)
                {
                fprintf(LDot, "%d [shape=record, label=\"{NodePointer:  | %x} | {ParentPointer: | %x} | {data: | %lf} | {Left: | Right:} | {%x | %x}\",style=\"filled\"];\n", NowNode, NowNode, NowNode->Parent, *(double*)NowNode->data, NowNode->Left, NowNode->Right);
                }
            else
                {
